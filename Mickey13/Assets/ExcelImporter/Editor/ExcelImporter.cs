...
public class ExcelImporter : AssetPostprocessor
{
	...
	static object CellToFieldObject(ICell cell, FieldInfo fieldInfo, bool isFormulaEvalute = false)
	{
		...
		switch(type)
		{
			...
			case CellType.Numeric:
                if (fieldInfo.FieldType.Name == "BigNumericWrapper")
                {
                    double rawValue = cell.NumericCellValue;
                    object bigNum = Activator.CreateInstance(fieldInfo.FieldType, rawValue);
                    return bigNum;
                }
	...
	static object CreateEntityFromRow(IRow row, List<string> columnNames, Type entityType, string sheetName)
	{
		...
		for (int i = 0; i < columnNames.Count; i++)
		{
            ...
            if (entityField == null)
            {
                Debug.LogWarning($"[ExcelImporter] '{columnNames[i]}' 열에 대응되는 필드가 {entityType.Name}에 없습니다.");
                continue;
            }
            ...
            if (cell == null)
            {
                Debug.LogWarning($"[ExcelImporter] row={row.RowNum}, col={i}, '{columnNames[i]}' 셀이 비어있음");
                continue;
            }
	...
	static void ImportExcel(string excelPath, ExcelAssetInfo info)
	{
		...
		foreach (var assetField in assetFields)
		{
            ...
            if (sheet == null)
            {
                Debug.LogWarning($"[ExcelImporter] 시트를 찾지 못함: 필드명='{assetField.Name}'");
                continue;
            }
            else
            {
                Debug.Log($"[ExcelImporter] 매칭됨: 필드명='{assetField.Name}' ↔ 시트명='{sheet.SheetName}'");
            }
